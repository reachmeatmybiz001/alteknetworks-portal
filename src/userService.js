...
export function updateUser(username, changes = {}) {
  if (changes.enabled === true) {
    return apiUpdateUser(username, { action: 'enable' })
  }

  if (changes.enabled === false) {
    return apiUpdateUser(username, { action: 'disable' })
  }
...
export function isAdministratorRole(role) {
  return ['SupportAdmins', 'SuperAdmins'].includes(role)
}

export function canManageUserRole(actorRole, targetRole) {
  if (actorRole !== 'SuperAdmins') return false
